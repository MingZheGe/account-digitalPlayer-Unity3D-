...
export default {
    "bizProCreditRecordInfoBeforeLoadBiz": (_this) => {
        let saveGroupId = _this.userType == "0" ? "CUST_INFO" : "ORG_INFO";
    ...
    "bizProCreditRecordInfoBeforeSave": (_this, params) => {
        ...
        let originaCreditRecordInfoFields = _this.groupDatas.CUST_INFO.CREDIT_RECORD_INFO[0].FIELDS;
        ...
        if (!creditRecordInfo.length) {
            let findItem = _.find(originaCreditRecordInfo, {RECORD_SOURCE: "07"});
            if (!_.isEmpty(findItem)) {
                creditRecordInfo.push(findItem);
            } else {
        ...
        if (_this.busiCode == "V0049" || _this.busiCode == "V0163") {
            ...
            Object.assign(params, {
                CUST_CREDIT_RECORD: relaInfo.CREDIT_RECORD_CHANGE_INFO.INFO
            });
        }
    ...
    CREDITRECORDINFO__DELETE_MODULE_FINISHED: (_this, deleteObj) => {
        _this.$nextTick(() => {
            let saveGroupId = _this.userType == "0" ? "CUST_INFO" : "ORG_INFO";
            let CREDIT_RECORD_INFO = _this.groupDatas[saveGroupId].CREDIT_RECORD_INFO;
