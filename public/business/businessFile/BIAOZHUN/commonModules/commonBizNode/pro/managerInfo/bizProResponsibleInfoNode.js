...
const backfillHistoryData = (_this) => {
    let bizProResponsibleInfo = _this.historyData["ORG_RESPONSIBLE_INFO"];
    if (!_.isEmpty(bizProResponsibleInfo)) {
        bizPublicMethod.$blMethod.parseGroupsMouduleData(_this, 
            ["RESPONSIBLE_PERSON_INFO"], bizProResponsibleInfo);
        let module = _this.groupDatas[GROUP_ID].RESPONSIBLE_PERSON_INFO[0];
...
export default {
    //----------------------------------钩子函数----------------------//
    /*
     *@method bizProResponsibleInfoBeforeLoadBiz
     *@desc 钩子函数 加载历史数据之前触发
     */
    bizProResponsibleInfoBeforeLoadBiz: function (_this) {
        let bizProResponsibleInfo = custInfoModel.getOriginaOrgResponsibleInfo(_this.oppBusiData.oldBusiData);
        if (!_.isEmpty(bizProResponsibleInfo)) {
            bizPublicMethod.$blMethod.parseGroupsMouduleData(_this, ["RESPONSIBLE_PERSON_INFO"], bizProResponsibleInfo);
        }
    },
    /*
     *@method bizProResponsibleInfoAfterLoadBiz
     *@desc  钩子函数  加载历史数据后触发
     */
    bizProResponsibleInfoAfterLoadBiz: function (_this) {
        backfillHistoryData(_this);
        let allIdTypes = _.union(_this.oppBusiData.individualValidIdTypes);
        bizPublicMethod.$blMethod.filterOptionsButRetainHistoryOptions(_this, GROUP_ID, "RESPONSIBLE_PERSON_INFO",
    ...
    /*
     *@method bizProResponsibleInfoBeforeSave
     *@desc 钩子函数 自定义保存数
     */
    bizProResponsibleInfoBeforeSave: function (_this, params) {
        ...
        if (_this.busiCode == "V0051") {
            ...
            managerInfo.ORG_RESPONSIBLE_INFO = params.ORG_RESPONSIBLE_INFO;
            delete managerInfo.ORG_RESPONSIBLE_INFO.DIFF_INFO;
            // 翻译字段字典项，以便影像展示
            let fields = _this.groupDatas[GROUP_ID].RESPONSIBLE_PERSON_INFO[0].FIELDS;
            managerInfo.ORG_RESPONSIBLE_INFO.RESPONSIBILITY_REP_ID_TYPE_TEXT = 
        ...
        //增加diffinfo 属性 用于展示
        let responsiblePersonFields = _this.groupDatas[GROUP_ID].RESPONSIBLE_PERSON_INFO[0].FIELDS;
    ...
    /*
     *@method bizProResponsibleInfoAfterSave
     *@desc 钩子函数 自定义保存数
     */
    bizProResponsibleInfoAfterSave: (_this, newData) => {

    },
    /*
     *@method bizProResponsibleInfoValidate
     *@desc 钩子函数 下一步验证
     */
    bizProResponsibleInfoValidate: function (_this) {
        let modules = _this.groupDatas[GROUP_ID].RESPONSIBLE_PERSON_INFO;
    ...
    /*
     *@method bizProResponsibleInfoPageActivated
     *@desc 钩子函数：页面激活
     */
    bizProResponsibleInfoPageActivated: function (_this) {
        let legalRepType = 
            _this.groupDatas.MANAGER_INFO.MANAGER_BASIC_INFO[0].FIELDS.LEGAL_REP_TYPE.DEFAULT_VALUE;
        _this.groupDatas[GROUP_ID].RESPONSIBLE_PERSON_INFO[0].FIELDS.MODULE_RADIO_BUTTON.FIELD_TITLE = 
            legalRepType == "08" ? "与执行事务合伙人信息一致" : "与法定代表人信息一致";
    },

    bizProResponsibleInfoPreValidate: function (_this) {},
    //----------------------业务函数----------------------------------//
    ...
    "CHECK_RESPONSIBILITY_REP_ID_EXP_DATE": (_this, field, fieldData, module) => {
