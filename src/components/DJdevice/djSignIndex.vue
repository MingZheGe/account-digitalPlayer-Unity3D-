<template>
    <div class="mask-wrap dj-sign" :style="{ left: formSignLeft }">
...
export default {
  ...
  computed: {
     ...
     */
    formSendObj() {
        return this.$store.state.formSendObj;
    }
  },
  mounted() {
    this.startSign();
  ...
  watch: {
    ...
    showFormSign (v) {
        this.formSignLeft = v ? '0%' : "-100%"
    },
    formSendObj(val) {
        if (!_.isEmpty(val)) {
            if (val.func == "showSign") {
                this.showSign(val);
            } else if(val.func == "showNotation") {
              this.showNotation(val)
            }
        }
    }
  },
  methods: {
        ...
        if (item.imageArr && item.imageArr.length) {
          for(let i = 0; i < item.imageArr.length; i++) {
            ...
            imageMid.onload = function() {
              signOverNum++
              ctx.drawImage(imageMid, 0, 0, imageMid.naturalWidth - 1, imageMid.naturalHeight - 1, item.signStartArr[i] + 24, lvlHeight[index], imageMid.naturalWidth * 0.07, imageMid.naturalHeight * 0.07);
    ...
    initAipSign() {
      ...
      try {
        that.djSignInstance = djSignObj.initAndOpenAipSign("888888");
    ...
    showSign(val) {
        this.isNeedVideo = val.isNeedVideo;
        this.isNotation = false;
        this.notationContent = "";
        if (this.isNeedVideo) {
            this.messageBox({
                typeMessage: 'warn',
    ...
    showNotation(val) {
      ...
      this.notationContentLabel = [];
      this.notationContentText =  "";
      this.notationSignArr =  [];
      this.notationCurrentIndex =  0;
      this.notationSignLen =  0;
      this.nextBtnText =  "下一页";
      this.notationContent = val.notationContent;
      // 将其偏移到正中间
      this.showFormSign = true;
      this.setNotation();
    ...
    closeMask () {
        ...
        this.$store.commit(this.$types.SIGN_OVER_OBJ, {
            "func": "closeMask"
        });
    },
    // 清空签名数据
    ...
    signNext() {
      if (this.nextBtnText == "提交") {
        ...
        this.showFormSign = false;
        this.djSignInstance.clearSign();
        this.$store.commit(this.$types.FORM_SEND_OBJ, {});
        this.$store.commit(this.$types.SIGN_OVER_OBJ, {
            "func": "notationOver",
            "signvaltag": signImage,
        });
      } else {
        this.signSave()
      }
    ...
    signOk() {
      ...
      that.djSignInstance.getSignTrack().then((signTrackVal) => {
        ...
        if (
            ...
            this.$store.commit(this.$types.SIGN_OVER_OBJ, {
                "func": "signOkLoading",
                "signvaltag": that.signvaltag,
                "signTrackVal": signTrackVal
            });
        } else {
            that.messageBox({
                confirmButtonText: "确定",
