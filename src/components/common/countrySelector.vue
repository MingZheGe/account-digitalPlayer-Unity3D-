<template>
  <div
    ...
    <input
      ...
      :class="'k-country__input--' + size"
      class="k-country__input"
      type="text"
      autocomplete="off">
    <div
      ref="panel"
      @click.stop=""
      class="k-country__panel">
      <div class="k-country__select" v-show="selectShow">
      ...
      <ul class="k-country__search" v-show="searchShow">
        <li
...
<script>
  import countryData from '../preEntry/countryselect-data'
  ...
  export default {
    ...
    watch: {
      'select.text': function (newVal, oldVal) {
        if (newVal !== oldVal) {
    ...
    methods: {
      ...
      getCode(name) {
        for (let i = 0, j = this.data.length; i < j; i++) {
          ...
          let country = this.data[i];
          if (this.lang === 'zh') {
            if(country.cnName === name) {
              this.select.text = name;
...
<style lang="less" scoped>
  ...
  .k-country {
    ...
    .k-country__input--large {
      height: 40px;
      line-height: 40px;
      font-size: 14px;
    }
    
    .k-country__input--medium {
      height: 36px;
      line-height: 36px;
      font-size: 14px
    }
    
    .k-country__input--small {
      height: 32px;
      line-height: 32px;
      font-size: 13px;
    }
    
    .k-country__input--mini {
      height: 28px;
      line-height: 28px;
      font-size: 12px;
    }
