...
import V0049_orgControllerTaxInfo from "./V0049_orgControllerTaxInfo.vue"
...
export default {
            ...
            infoList: [
                {
                    ...
                    name: "证件信息",
                    desc: "机构名称、证件类型、证件号码、证件有效期等",
                    isShowType: "0",  // 0：不需要修改  1：待完善  2：已修改 3：驳回
                    ygt_module: "ORG_INFO_MODIFY_CARD" //业务审核模块配置(对应一柜通的审核模块)
                },
                {
                    ...
                    name: "基本信息",
                    desc: "机构类别、行业类别、诚信记录信息等",
                    isShowType: "0",
                    ygt_module: "ORG_INFO_MODIFY"
                },
                {
                    img_blue: require("../../../../icons/openList/ic/icon-open-contact.svg"),
                    img_grad: require("../../../../icons/openList/ic/icon-open-contact-gray.svg"),
                    name: "联系信息",
                    desc: "手机号码、固定电话、办公地址、联系人信息等",
                    isShowType: "0",
                    ygt_module: "ORG_INFO_MODIFY_LINK"
                },
                {
                    ...
                    name: "关联人信息",
                    desc: "法定代表人/执行事务合伙人名称、证件信息等",
                    isShowType: "0",
                    ygt_module: "ORG_INFO_MODIFY_RELA"
                },
                {
                    ...
                    desc: "名称、证件信息等",
                    isShowType: "0",
                    ygt_module: "ORG_INFO_MODIFY_CONTROL"
                },
                {
                    ...
                    desc: "姓名、证件信息等",
                    isShowType: "0",
                    ygt_module: "ORG_INFO_MODIFY_BENEFICIARY"
                },
                {
                    ...
                    ygt_module: "ORG_INFO_MODIFY_ADEQ"
                },
                {
                    img_blue: require("../../../../icons/openList/ic/icon-open-tax.svg"),
                    img_grad: require("../../../../icons/openList/ic/icon-open-tax-gray.svg"),
                    ...
                    desc: "控制人涉税居民身份信息等",
                    isShowType: "0",
                    isHide: true, // 是否隐藏整个信息
                    ygt_module: "ORG_INFO_MODIFY_CTAX"
                }
            ]
        }
    },
    ...
    created() {
        ...
        Promise.all([
            ...
            newInfoList = that.setControllerTaxInfo(newInfoList)
    ...
    activated() {
        ...
        newInfoList = this.setControllerTaxInfo(newInfoList);
    ...
    watch: {
        ...
        bizRouteTable(val) {
            this.currentInfoList = this.setControllerTaxInfo(this.currentInfoList)
        }
    },
    methods: {
        //设置控制人涉税是否隐藏
        setControllerTaxInfo (newInfoList) {
            //判断涉税是否隐藏
            let newInfoListTpl = _.cloneDeep(newInfoList);
            let bizRouteTableTpl = _.cloneDeep(this.bizRouteTable);
            let controllerTaxInfoRoute = _.find(bizRouteTableTpl, {name: '控制人涉税声明信息'});
            newInfoListTpl = _.map(newInfoListTpl, item => {
                if (item.name == "控制人涉税信息") {
                    item.isHide = !controllerTaxInfoRoute.show;
                }
                return item;
            })
        ...
        updateIsshowType (info, moduleIdArr) {
            ...
            if (groupId == "CONTROLLER_TAX_INFO") {
                return this.updateControlTaxInfoShowType();
            }
                ...
                _.each(moduleIdArr, moduleId => {
                    ...
                    _.each(newModuleData, moduleInfo => {
                        ...
                        _.each(moduleInfo.FIELDS, field => {
                            ...
                            // 特例：受益所有人的地址
                            if (moduleInfo.MODULE_CONTROL == "1" && moduleInfo.MODULE_ID == "BENEFICIARY_OWNER_INFO" && field.FIELD_TYPE == "addressInput") {
                                let idType = _.get(moduleInfo, "FIELDS.BENEFICIARY_ID_TYPE.DEFAULT_VALUE", "");
                                if (_this.busiLogic["checkBeneficiaryAddress"] && 
                                !_this.busiLogic["checkBeneficiaryAddress"](idType, field.DEFAULT_VALUE)) {
                                    flag = false;
                                    return false;
                                }
                            }
            ...
            // 特例：控股股东、控制人、受益人证件类型重复时显示待完善
            if (moduleIdArr.includes("STOCKHOLDER_INFO") || moduleIdArr.includes("ACTUAL_CONTROLLER_INFO") ||
            moduleIdArr.includes("BENEFICIARY_OWNER_INFO")) {
                for (let i in moduleIdArr) {
                    let moduleId = moduleIdArr[i];
                    let fieldId = "";
                    if (moduleId == "STOCKHOLDER_INFO") {
                        fieldId = "CONTROLLER_ID_CODE";
                    } else if (moduleId == "ACTUAL_CONTROLLER_INFO") {
                        fieldId = "CONTROLER_ID_NO";
                    } else if (moduleId == "BENEFICIARY_OWNER_INFO") {
                        fieldId = "BENEFICIARY_ID";
                    }
                    if (fieldId != "" && bizPublicMethod.$blMethod.isFieldDuplicated(this, groupId, 
                    moduleId, fieldId)) {
                        isShowType = "1";
                        return isShowType;
                    }
        ...
        //控制人涉税状态更新
        updateControlTaxInfoShowType () {
            let controllerTaxInfo  = _.cloneDeep(this.oppBusiData.currentControllerTaxInfo);
            let infoVue = V0049_orgControllerTaxInfo;
            controllerTaxInfo = infoVue.methods.updateControlTaxInfoArr(controllerTaxInfo, this.groupDatasTpl.CONTROLLER_TAX_INFO);
            let change = _.cloneDeep(this.historyData.controllerTaxInfoArrChange) || {};
            let oneShowType = _.find(controllerTaxInfo, {showType: "1"});
            if (!_.isEmpty(oneShowType) || _.isEmpty(controllerTaxInfo)) {
                return "1";
            }
            let showType = "0"
            _.each(change.WEB_SHOW, item => {
                if (item.showDiffInfo.length || item.deleteDiff.length) {
                    showType = "2";
                    return false;
                }
            })
        ...
        clickItem(index){
            switch(index) {
                ...
                break;
                case 3: {
                    let legalRepType = this.groupDatas.ORG_INFO.SUBJECT_IDENTITY_INFO[0].FIELDS.LEGAL_REP_TYPE.DEFAULT_VALUE;
                    let szOrgType = this.groupDatas.ORG_INFO.SUBJECT_IDENTITY_INFO[0].FIELDS.SZORG_TYPE.DEFAULT_VALUE;
                    if (_.isEmpty(legalRepType) || _.isEmpty(szOrgType)) {
                        this.$parent.messageBox({
                            hasMask: true,
                            messageText: "您的机构类别/法人类别信息不完善，请先选择【基本信息】模块进行完善",
                ...
                break;
                case 4: {
                    this.$router.goRoute('控股股东/实际控制人信息')
                }
                break;
                case 5: {
            ...
            if (!_.isEmpty(originalBasicInfo)) {
                ...
                if (amlLvl == "2") {
                    let isKeyInfoChanged = 
                        ...
                        basicInfo.ORG_ID_CODE != originalBasicInfo.ORG_ID_CODE;
                    if (isKeyInfoChanged) {
                        _this.oppBusiData.flowTurnflag = "1";
                    }
                }
            }
            _this.$refs.V0049_ORG.busiLogic.bizOrgTaxInfoNodeValidate(_this);
        },
        //获取变更后的一账通信息
        ...
        beforeSave(_this, params) {
            return Promise.all([
                _this.$refs.V0049_ORG.busiLogic.bizOrgBasicInfoNodeBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgCreditRecordInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgLinkInfoNodeBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrglinkmanInfoNodeBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgLegalRepInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgLegalClientInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgResponsibleInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgPartnerInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgStockholderInfoBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgActualControllerNodeBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgBeneficiaryOwnerBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgTaxInfoNodeBeforeSave(_this, params),
                _this.$refs.V0049_ORG.busiLogic.bizOrgControllerTaxInfoNodeBeforeSave(_this, params, true),
            ]).then( async res => {
                // 流转条件
                ...
                //涉税信息处理
                this.detailTaxInfo(_this, params);
                //增加代理人信息
        ...
        //涉税信息保存处理
        detailTaxInfo(_this, params) {
            let taxInfoArrChange = params.taxInfoArrChange;
            let controllerTaxInfoArrChange = params.controllerTaxInfoArrChange;
            let orgTaxInfo = {};
            orgTaxInfo.INFO = _.concat([], _.cloneDeep(taxInfoArrChange.INFO), _.cloneDeep(controllerTaxInfoArrChange.INFO));
            orgTaxInfo.IS_CHANGE = (taxInfoArrChange.IS_CHANGE == "1" || controllerTaxInfoArrChange.IS_CHANGE == "1") ? "1" : "0"
            let orgTaxInfoItem = _.concat([], _.cloneDeep(taxInfoArrChange.orgTaxInfoItem), _.cloneDeep(controllerTaxInfoArrChange.orgTaxInfoItem));
            let taxParams = {
                ORG_TAX_INFO: orgTaxInfo.INFO || [],
                TAX_INFO_FLAG: orgTaxInfo && orgTaxInfo.IS_CHANGE || "1",  //华林、联讯：涉税如果是新增或修改需要动态显示税收居民声明表,0 表示原来的值没有改变，1表示值已变
                ORG_TAX_INFO_ITEM: orgTaxInfoItem,
                IS_CHANGE_ORG_TAX: taxInfoArrChange.IS_CHANGE == "1" ? "1" : "0",
                IS_CHANGE_CTL_TAX: controllerTaxInfoArrChange.IS_CHANGE == "1" ? "1" : "0",
                IS_SHOW_TAX_FLAG: "1"
            }
            Object.assign(params, taxParams);
        },
        /**
         * 同步信息数据处理
         */
        async setSyncSys(_this, params) {
            ...
            let CSDC_YMT_CODE = "";
            let CUST_BASIC_INFO = params.ORG_BASIC_INFO;
            //一码通处理
        ...
        //变更相关信息（名称、证件类型、证件号码、组织机构代码证、手机号码、证件结束日期、法人名称、法人证件号码、联系人姓名、联系人证件类型、联系人证件号码、联系地址） 同步需要同步中登 展示证券账户业务申请表
        setCsdcChangeInfo(params) {
            params.CHANGE_CSDC_FLAG = "0";
            let csdcChangeInfo = ["USER_FNAME", "ID_TYPE", "ID_CODE", "ID_EXP_DATE", "ORG_ID_CODE", "MOBILE_TEL", "LEGAL_REP", "LEGAL_REP_ID_CODE", "LINKMAN_NAME", "LINKMAN_ID", "OFFICE_ADDR"];
            let change = [];
            let basicChnage = _.get(params, "ORG_BASIC_INFO.DIFF_INFO", []);
            let importInfo = _.get(params, "ORG_IMPORT_INFO.DIFF_INFO", []);
            let linkInfo = _.get(params, "ORG_LINK_INFO.DIFF_INFO", []);
            let legalInfo = _.get(params, "ORG_LEGAL_REP_INFO.DIFF_INFO", []);
            let linkManInfo = [];
            _.each(_.get(params, "ORG_LINKMAN_INFO.INFO", []), linkManInfoItem => {
                if (linkManInfoItem.OPER_TYPE != "2") {
                    linkManInfo = _.concat(linkManInfo, _.get(linkManInfoItem, "DIFF_INFO", []))
                }
                if (linkManInfoItem.OPER_TYPE == "2") {
                    params.CHANGE_CSDC_FLAG = "1";   
                }
            })
            change = _.concat([], importInfo, basicChnage, legalInfo, linkInfo);
        ...
        setCheckCsdcAndYMT(_this, params) {
            ...
            // 1. 系统内无证券账户 
            // 2. 特殊机构户
            // 3. 非特殊机构，系统内有证券账户，但未修改名称、简称、证件类型、证件号码、证件结束日期、注册地址、组织机构代码、组织机构代码证有效期、机构类别、
            //                                      资本属性、法定代表人名称、执行事务合伙人名称、法定代表人证件类型、执行事务合伙人证件类型、法定代表人证件号码、
            //                                      执行事务合伙人证件号码、联系人姓名、联系人证件类型、联系人证件号码、办公地址、私募基金管理人编码
            let csdcYmtCheckInfo = ["USER_FNAME", "USER_NAME", "ID_TYPE", "ID_CODE", "ID_EXP_DATE", "CORP_ADDR", "ORG_ID_CODE", "ORG_ID_EXP_DATE", "SZORG_TYPE", 
                "CAPITAL_ATTR", "LEGAL_REP", "LEGAL_REP_ID_TYPE", "LEGAL_REP_ID_CODE", "LINKMAN_NAME", "LINKMAN_ID_TYPE", "LINKMAN_ID", "OFFICE_ADDR", "SMFUND_MANAGER_ID"];
            ...
            let customerInfo = _this.$storage.getJsonSession(_this.$definecfg.CUSTOMER_INFO);
            let szorgType = customerInfo.SZORG_TYPE;
            let isSpecialSzorg = false;
            if (params.ORG_BASIC_INFO.SZORG_TYPE == szorgType && specialSzorgTypes.indexOf(szorgType) != -1) {
                isSpecialSzorg = true;
            }
            let sysTrdacct = _this.oppBusiData.oldBusiData.SYS_TRDACCT;
            if (isSpecialSzorg || _.isEmpty(sysTrdacct) || (!_.isEmpty(sysTrdacct) && change.length == 0)) {
                IS_CHECK_7_24 = "0";
            }
            // 是否检查7*24小时任务
