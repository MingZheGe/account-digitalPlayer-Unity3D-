import bizTaxMethod from '../../../../businessTools/bizTaxMethod.js'
...
export default  {
   ...
   custTaxInfoModuleBeforeLoadBiz: function(_this){
        ...
        oppData.BIRTH_ADDRESS1 = _this.oppBusiData.custTaxInfo.BIRTH_ADDRESS;
        if(oppData.CURR_CODE == "CNY"){
            oppData.CURR_CODE = "RMB";
        }
...
/*
   ...
   custTaxInfoModuleAfterLoadBiz: function(_this){
    let custTaxInfo =  _this.historyData.CUST_TAX_INFO && _this.historyData.CUST_TAX_INFO.length && _this.historyData.CUST_TAX_INFO[0] || {};
         ...
         if (_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE1[0].FIELDS.TAX_RESIDENT_TYPE.DEFAULT_VALUE === "1") {
             ...
             _this.$blMethod.setModulesControl(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE5, "0")
             _this.$blMethod.setModulesControl(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE7, "0")
         } else {
             //显示关联模块
             ...
             _this.$blMethod.setModulesControl(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE5, "1")
             _this.$blMethod.setModulesControl(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE7, "1")
         }
   ...
   getCustTaxInfoData : function(_this,params) {
        //这里由于存在使用的可能是BIRTH_ADDRESS1 需要将值赋值给 BIRTH_ADDRESS
        let birthAddress = _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE4[0].FIELDS.BIRTH_ADDRESS1.DEFAULT_VALUE;
        let birthAddressControl = _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE4[0].FIELDS.BIRTH_ADDRESS1.FIELD_CONTROL;
        if(birthAddressControl == "0"){
            _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE4[0].FIELDS.BIRTH_ADDRESS.DEFAULT_VALUE = birthAddress;
        }
        //手动转换为VTM数据
        let saveModel = ["CUST_TAX_INFO_MODULE1","CUST_TAX_INFO_MODULE2","CUST_TAX_INFO_MODULE3","CUST_TAX_INFO_MODULE4"];
        ...
        if(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE5[0].MODULE_CONTROL == "1"){
            _.forEach(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE5, function(item){
                item.FIELDS.CITIZENSHIP = item.FIELDS.CITIZENSHIP_ST;
                delete item.FIELDS.CITIZENSHIP_ST;
            })
        ...
        saveTaxInfo = bizPublicSaveMethod.getSaveMonamntData(_this, saveTaxInfo, _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE6, _this.oppBusiData.custTaxInfo);
        if(_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE7[0].MODULE_CONTROL == "1"){
            saveTaxInfo = bizPublicSaveMethod.getSavePaymentAssetData(_this,saveTaxInfo,_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE7,_this.oppBusiData.custTaxInfo);
        }
        ...
        let custTaxInfoDiff = _this.$blMethod.getArrDiff([saveTaxInfo], custTaxInfo, "CTRL_FLAG", "OPP_NO_TAXPAYERID_REASON");
        
        // 如果不显示涉税信息 或 选择了“仅为中国税收居民”，需要删除存量的居民国信息
        let custTaxInfoItem = _this.$blMethod.getArrDiff(
            saveTaxInfo && saveTaxInfo.TAX_RESIDENT_TYPE != "1" ? _this.$blMethod.transTaxInfoToArr(saveTaxInfo) : [],
            !_.isEmpty(custTaxInfo) ? _this.$blMethod.transTaxInfoToArr(custTaxInfo[0]) : [],
            "TAX_ID", "", true
        );

        // 如果不显示涉税信息 或 选择了“仅为中国税收居民”，需要删除存量的居民国资产信息
        let custTaxAssetInfoItem = _this.$blMethod.getArrDiff(
            saveTaxInfo && saveTaxInfo.TAX_RESIDENT_TYPE != "1" ? _this.$blMethod.transTaxAssetInfoToArr(saveTaxInfo) : [],
            !_.isEmpty(custTaxInfo) ? _this.$blMethod.transTaxAssetInfoToArr(custTaxInfo[0]) : [],
            "TAX_ID", "", true
        );
        ...
        let CUST_TAX_INFO = {"CUST_TAX_INFO":[custTaxInfoItem.TAX_INFO[0]]};
   ...
   custTaxInfoModuleParseOldBiz :function (_this, bdata) { 
        _this.$blMethod.parseGroupsMouduleData(_this,["CUST_TAX_INFO_MODULE1",
                                                    "CUST_TAX_INFO_MODULE2",
                                                    "CUST_TAX_INFO_MODULE3",
                                                    "CUST_TAX_INFO_MODULE4",
                                                    "CUST_TAX_INFO_MODULE6"],bdata);

        _this.$blMethod.parseHistoryTaxWithTaxPayerData(_this,"TAX_INFO","CUST_TAX_INFO_MODULE5",bdata);
        _this.$blMethod.parseHistoryTaxWithPayMentTaxData(_this,"TAX_INFO","CUST_TAX_INFO_MODULE7",bdata);
    ...
    //--------------------------------------------------检查逻辑--------------------------------------------------
     ...
     "CHECK_TAX_FLAG" : (_this, field, fieldData) => {
        ...
        if(field.DEFAULT_VALUE === "" || field.DEFAULT_VALUE === undefined){
            ...
            //选择了仅中国居民和豁免
            let customerInfo = _this.$blMethod.getCustomerAllData(_this);
            ...
            if(field.DEFAULT_VALUE === "1" && _.indexOf(["0b","0c","0d","0e","0i","0j","0s"],customerInfo.ID_TYPE ) != -1){
                ...
                _this.$router.getCurrentRoute().nextBtnText = "填写完成";
            ...
            _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE2[0].MODULE_CONTROL = "1";
            if(bizTaxMethod.check_Module_Radio_flag3(_this,_this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE2[0].FIELDS)){
                _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE2[0].FIELDS.MODULE_RADIO_BUTTON.DEFAULT_VALUE = true;
            }
            //显示关联模块
            _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE3[0].MODULE_CONTROL = "1";
            _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE4[0].MODULE_CONTROL = "1";
            if(!_this.$blMethod.isChinese(_this)){
                _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE5[0].MODULE_CONTROL = "0" ;
            }else{
    ...
    "CHECK_MODULE_RADIO" : (_this, field, fieldData) => {
        if(_this.userType == "0"){
            if(field.DEFAULT_VALUE === "true"){
                let nameObj = stringConfig.getFirstName(_this.groupDatas.CUST_INFO.CUST_BASIC_INFO[0].FIELDS.USER_FNAME.DEFAULT_VALUE);//过滤姓
                ...
                if(stringConfig.isNotEmptyStr(_this.groupDatas.CUST_INFO.CUST_BASIC_INFO[0].FIELDS.ID_ADDR.DEFAULT_VALUE)){
                    _this.groupDatas.TAX_INFO.CUST_TAX_INFO_MODULE2[0].FIELDS.ADDRESS.DEFAULT_VALUE = _this.groupDatas.CUST_INFO.CUST_BASIC_INFO[0].FIELDS.ID_ADDR.DEFAULT_VALUE;
                }
            }else{
    ...
    "CHECK_OPP_NO_TAXPAYERID_REASON":(_this, field,fieldData) =>{
        if(field.DEFAULT_VALUE == "1"){
            fieldData.NO_TAXPAYERID_REASON.FIELD_CONTROL = "1";
            fieldData.NO_TAXPAYERID_REASON.DEFAULT_VALUE = field.FIELD_DICT_NAME[field.DEFAULT_VALUE-1].DICT_ITEM_NAME;
