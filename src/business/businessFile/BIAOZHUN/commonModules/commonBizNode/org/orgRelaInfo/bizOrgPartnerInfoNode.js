...
const backfillHistoryData = (_this) => {
    ...
    if( !_.isEmpty(partnerInfo)){
        bizPublicMethod.$blMethod.parseMoudleArray(_this, _this.groupDatas["RELA_INFO"]["PARTNER_INFO"], 
...
const isPartnerInfoChanged = (_this) => {
    ...
    // 如果模块被隐藏了，赋值为空
    if (_this.groupDatas.RELA_INFO.PARTNER_INFO[0].MODULE_CONTROL == "0") {
        partnerInfo = [];
    }
...
//开户和非开户 字段数据加载 公共操作
const bizOrgPartnerInfoBeforeLoadBizCommon = (_this) => {
    _this.groupDatas.RELA_INFO.PARTNER_INFO[0].MAX_LENGTH = "999";
    _this.groupDatasTpl.RELA_INFO.PARTNER_INFO[0] = _.cloneDeep(_this.groupDatas.RELA_INFO.PARTNER_INFO[0]);
}
const modifyFieldConfig = (_this) => {
    ...
    bizPublicMethod.$blMethod.filterOptionsButRetainHistoryOptions(_this, "RELA_INFO", "PARTNER_INFO",
     "PID_TYPE", allIdTypes, EXCLUDE_ID_TYPES, custInfoModel.getOriginaPartnerInfo(_this.oppBusiData.oldBusiData));
}
export default {
    //----------------------------------钩子函数----------------------//
    /*
     *@method bizOrgPartnerInfoBeforeLoadBiz
     *@desc 钩子函数 加载历史数据之前触发
     */
    bizOrgPartnerInfoBeforeLoadBiz: function (_this) {
        bizOrgPartnerInfoBeforeLoadBizCommon(_this)
    ...
    bizOrgPartnerInfoBeforeLoadBizOpenAcct: function (_this) {
        bizOrgPartnerInfoBeforeLoadBizCommon(_this)
    },
    /*
     *@method bizOrgPartnerInfoAfterLoadBiz
     *@desc  钩子函数  加载历史数据后触发
     */
    bizOrgPartnerInfoAfterLoadBiz: function (_this) {
        backfillHistoryData(_this);
        modifyFieldConfig(_this);
    },
    /*
     *@method bizOrgPartnerInfoBeforeSave
     *@desc 钩子函数 自定义保存数
     */
    bizOrgPartnerInfoBeforeSave: function (_this, params) {
        ...
        if (_this.busiCode == "V0050") {
            ...
            relaInfo.ORG_PARTNER_INFO = params.ORG_PARTNER_INFO;
            delete relaInfo.ORG_PARTNER_INFO.DIFF_INFO;
            relaInfo.ORG_PARTNER_INFO_FLAG = _.isEmpty(partnerInfo) ? false : true;
            Object.assign(params, {
                RELA_INFO: relaInfo
            })
        }
        //增加diffinfo 属性 用于展示
        let partnerInfoFields = _this.groupDatas.RELA_INFO.PARTNER_INFO[0].FIELDS;
    ...
    /*
     *@method bizOrgPartnerInfoAfterSave
     *@desc 钩子函数 自定义保存数
     */
    bizOrgPartnerInfoAfterSave: (_this, newData) => {

    },
    /*
     *@method bizOrgPartnerInfoValidate
     *@desc 钩子函数 下一步验证
     */
    bizOrgPartnerInfoValidate: function (_this) {
    ...
    /*
     *@method bizOrgPartnerInfoPageActivated
     *@desc 钩子函数：页面激活
     */
    bizOrgPartnerInfoPageActivated: function (_this) {
        showPartnerInfoModuleIfNeeded(_this);
    },

    bizOrgPartnerInfoNodeAddModuleFinished: function (_this, module) {
        let allIdTypes = _.union(_this.oppBusiData.individualValidIdTypes, _this.oppBusiData.orgValidIdTypes);
        bizPublicMethod.$blMethod.filterOptionsButRetainHistoryOptions(_this, "RELA_INFO", "PARTNER_INFO",
        "PID_TYPE", allIdTypes, EXCLUDE_ID_TYPES, custInfoModel.getOriginaPartnerInfo(_this.oppBusiData.oldBusiData));
    ...
    bizOrgPartnerInfoNodeDeleteModuleFinished: function (_this, module) {
        showTipIfPartnerInfoChanged(_this);
    },

    bizOrgPartnerInfoPreValidate: function (_this) {},
    //----------------------业务函数----------------------------------//
