...
const backfillHistoryData = (_this) => {
    ...
    if (!_.isEmpty(orgLinkmanInfo)) {
        bizPublicMethod.$blMethod.parseMoudleArray(_this, 
            _this.groupDatas["LINK_INFO"]["LINKMAN_INFO"], _.filter(orgLinkmanInfo, item => {
...
const initFieldConfig = (_this) => {
    _this.groupDatas.LINK_INFO.LINKMAN_INFO[0].FIELDS.LINKMAN_NAME.VALID_TYPE = "length[1,16]";
    let maxLinkman = _.get(_this.oppBusiData, "busiCommonParams.MAX_LINKMAN", "");
    _this.groupDatas.LINK_INFO.LINKMAN_INFO[0].MAX_LENGTH = _.isEmpty(maxLinkman) ? "999" : maxLinkman;
    _this.groupDatasTpl.LINK_INFO.LINKMAN_INFO[0] = _.cloneDeep(_this.groupDatas.LINK_INFO.LINKMAN_INFO[0]);
}

const modifyFieldConfig = (_this) => {
    bizPublicMethod.$blMethod.filterOptionsButRetainHistoryOptions(_this, "LINK_INFO", "LINKMAN_INFO",
...
export default {
    //----------------------------------钩子函数----------------------//
    /*
     *@method bizOrglinkmanInfoNodeBeforeLoadBiz
     *@desc 钩子函数 加载历史数据之前触发
     */
    bizOrgLinkmanInfoNodeBeforeLoadBiz: function (_this) {
        initFieldConfig(_this);
        return Promise.all([
            
        ]).then(res => {
            let orgLinkmanInfo = getOldData(_this);
            !_.isEmpty(orgLinkmanInfo) && bizPublicMethod.$blMethod.parseMoudleArray(_this, _this.groupDatas["LINK_INFO"]["LINKMAN_INFO"], orgLinkmanInfo);
        })
    },
    /*
     *@method bizOrglinkmanInfoNodeAfterLoadBiz
     *@desc  钩子函数  加载历史数据后触发
     */
    bizOrglinkmanInfoNodeAfterLoadBiz: function (_this) {
        backfillHistoryData(_this);
        modifyFieldConfig(_this);
    },
    /*
     *@method bizOrglinkmanInfoNodeBeforeSave
     *@desc 钩子函数 自定义保存数
     */
    bizOrglinkmanInfoNodeBeforeSave: function (_this, params) {
        ...
        //增加diffinfo 属性 用于展示
        let linkmanInfoInfoFields = _this.groupDatas.LINK_INFO.LINKMAN_INFO[0].FIELDS;
        ...
        if (_this.busiCode == "V0050") {
            ...
            Object.assign(params, params.ORG_LINKMAN_INFO[0], {
                RELA_INFO: relaInfo,
                
                LINKMAN: params.ORG_LINKMAN_INFO[0].LINKMAN_NAME,
                LINKMAN_ID: params.ORG_LINKMAN_INFO[0].LINKMAN_ID,
                LINKMAN_ID_TYPE: params.ORG_LINKMAN_INFO[0].LINKMAN_ID_TYPE,
                LINKMAN_MOBILE_TEL: params.ORG_LINKMAN_INFO[0].LINKMAN_MOBILE_TEL,
            })
        }
        return params;
    },
    /*
     *@method bizOrglinkmanInfoNodeValidate
     *@desc 钩子函数 下一步验证
     */
    bizOrglinkmanInfoNodeValidate: function (_this) {
        let saveGroupId = "LINK_INFO";
    ...
    /*
     *@method bizOrglinkmanInfoNodeGetData
     *@desc 拼接数据
     *@MethodAuthor  yangyp
     *@Date: 2019-06-11 15:19:09
     */
    bizOrglinkmanInfoNodeGetData: function (_this, params) {
    
    },

    /*
     *@method bizOrglinkmanInfoNodePageActivated
     *@desc 钩子函数：页面激活
     */
    bizOrglinkmanInfoNodePageActivated: function (_this) {

       
    },


    bizOrglinkmanInfoNodeAddModuleFinished: function (_this, module) {
    ...
    bizOrglinkmanInfoNodeDeleteModuleFinished: function (_this, module) {
    },
