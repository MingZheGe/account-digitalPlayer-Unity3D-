...
const backfillHistoryData = (_this) => {
    ...
    if (!_.isEmpty(stockholderInfo)) {
        bizPublicMethod.$blMethod.parseMoudleArray(_this, 
            _this.groupDatas[GROUP_ID]["STOCKHOLDER_INFO"], _.filter(stockholderInfo, item => {
                return item.OPER_TYPE && item.OPER_TYPE != "2";
            }));
        _.each(_this.groupDatas[GROUP_ID].STOCKHOLDER_INFO, module => {
...
const bizProStockholderInfoBeforeLoadBizCommon = (_this) => {
    _.each(_this.groupDatas[GROUP_ID].STOCKHOLDER_INFO, obj => {
        let fields = obj.FIELDS;
        fields.CONTROLLER.VALID_TYPE = "length[1,64]";
    })
    _this.groupDatas[GROUP_ID].STOCKHOLDER_INFO[0].MAX_LENGTH = "999";
    _this.groupDatasTpl[GROUP_ID].STOCKHOLDER_INFO[0] = _.cloneDeep(_this.groupDatas[GROUP_ID].STOCKHOLDER_INFO[0]);
}
...
const modifyFieldsConfig = (_this) => {
    ...
    bizPublicMethod.$blMethod.filterOptionsButRetainHistoryOptions(_this, GROUP_ID, "STOCKHOLDER_INFO",
...
export default {
    ...
    bizProStockholderInfoBeforeLoadBiz: function (_this) {
        ...
        if (stockholderInfo && stockholderInfo.length) {
            bizPublicMethod.$blMethod.parseMoudleArray(_this, _this.groupDatas[GROUP_ID]["STOCKHOLDER_INFO"], stockholderInfo);
        }
    },
    ...
    bizProStockholderInfoBeforeSave: function (_this, params) {
        ...
        if (_this.busiCode == "V0051") {
            ...
            managerInfo.ORG_STOCKHOLDER_INFO = params.ORG_STOCKHOLDER_INFO;
            delete managerInfo.ORG_STOCKHOLDER_INFO.DIFF_INFO;
            // 翻译字段字典项，以便影像展示
            for (let i in managerInfo.ORG_STOCKHOLDER_INFO) {
                let fields = _this.groupDatas[GROUP_ID].STOCKHOLDER_INFO[i].FIELDS;
                managerInfo.ORG_STOCKHOLDER_INFO[i].CONTROLLER_ID_TYPE_TEXT = 
        ...
        //增加diffinfo 属性 用于展示
        let stockholderInfoFields = _this.groupDatas[GROUP_ID].STOCKHOLDER_INFO[0].FIELDS;
    ...
    bizProStockholderInfoNodeAddModuleFinished: function(_this, module) {
        modifyFieldsConfig(_this);
        modifyRadioButtonTitleAccordingToLegalRepType(_this, module);
        updateRadioButtonsFieldControl(_this, module.GROUP_ID, module.MODULE_ID);
        module.FIELDS.CONTROLLER.VALID_TYPE = "length[1,64]";
    },

    bizProStockholderInfoNodeDeleteModuleFinished: function(_this, module) {
        updateRadioButtonsFieldControl(_this, module.GROUP_ID, module.MODULE_ID);
    },
    ...
    "CHECK_CONTROLLER_ID_EXP_DATE": (_this, field, fieldData, module) => {
